...
namespace FoodOrderingSystem.Services.Implementations
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public AuthenticationService(UserManager<User> userManager, SignInManager<User> signInManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration, IMailKitEmailService emailService, IImageUploadService imageUploadService, IMapper mapper, JwtHandler jwtHandler)
        {
            _mapper = mapper;
        ...
        {
            var userExists = await _userManager.FindByNameAsync(model.UserName.ToLower());
            ...
            if (model.Image != null)
            {
                ...
                var imageUrl = await _imageUploadService.PhotoUpload(model.Image);
        ...
        public async Task<bool> ResetPassword(ResetPasswordDto model)
        {
            ...
            if (!await _userManager.IsEmailConfirmedAsync(user))
                throw new ApplicationException("Account not confirmed");
